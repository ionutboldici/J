...
namespace J100.OutputProcessors
{
    public class Eolp
    {
        ...
        public Eolp(string outputPath, LogFilterService logFilterService)
        {
        ...
        public void GenerateReport(List<string> logFiles)
        {
            try
            {
                ...
                {
                    Console.WriteLine("[ERROR]: Nu există fișiere de log pentru a genera raportul EOL.");
                    return;
                }

                string outputFileName = $"EOL_{DateTime.Now:yyyy-MM-dd_HH-mm-ss}.xlsx";
                ...
                {
                    var worksheet = package.Workbook.Worksheets.Add("RaportEOL");
                    int row = 1;
                ...
                Console.WriteLine($"[INFO]: Raportul EOL a fost salvat în: {fullOutputPath}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ERROR]: Eroare la generarea raportului EOL: {ex.Message}");
            }
        }
    }
}
